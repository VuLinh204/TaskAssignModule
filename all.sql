...
BEGIN
    ...
    GROUP BY H.HeaderID, H.HeaderTitle, H.StartDate, H.PersonInCharge
    ...
    SELECT
        ...
        ISNULL((
            SELECT STUFF((
                ...
                WHERE AH2.TaskID=T.TaskID AND AH2.HeaderID=AH.HeaderID
                FOR XML PATH('')
            ),1,1,'')
    ...
    INNER JOIN tblTask_AssignHeader H ON H.HeaderID = AH.HeaderID
    WHERE ',' + AH.EmployeeID + ',' LIKE '%,' + @EmployeeID + ',%'
      AND T.Status=1
    ...
    SELECT
        ...
        ISNULL((
            SELECT STUFF((
                ...
                WHERE H2.TaskID=T.TaskID AND H2.HeaderID IS NULL
                FOR XML PATH('')
            ),1,1,'')
        ),'') AS AssignedToEmployeeIDs,
        (SELECT ParentTaskID FROM tblTask_Template WHERE ChildTaskID=T.TaskID) AS ParentTaskID
...
BEGIN
    ...
    -- Xác định HeaderID nếu có
    DECLARE @ExistingHeaderID INT = NULL;
    SELECT TOP 1 @ExistingHeaderID = HeaderID
    FROM tblTask_AssignHistory
    WHERE TaskID = @ChildTaskID AND HeaderID IS NOT NULL;

    -- Nếu đã có assign history cho task, update luôn
    IF EXISTS (SELECT 1 FROM tblTask_AssignHistory WHERE TaskID = @ChildTaskID)
    BEGIN
    ...
    SELECT 1 AS Success, N'Cập nhật người phụ trách thành công (1 dòng duy nhất)!' AS Message;
END
GO
...
BEGIN
    ...
    -- 3. Tính StartDate / EndDate trong ngày AssignmentDate
    DECLARE @StartDate DATETIME = CAST(@AssignmentDate AS DATETIME) + '00:00:01';
    DECLARE @EndDate DATETIME = CAST(@AssignmentDate AS DATETIME) + '23:59:59';
    ...
    WHERE TaskID = @ParentTaskID
      ...
      AND CAST(StartDate AS DATE) = @AssignmentDate;
    ...
    BEGIN
        SELECT TOP 1 @NewHeaderID = HeaderID
        FROM tblTask_AssignHistory
        WHERE TaskID = @ParentTaskID
          AND EmployeeID = @MainResponsibleID
        ...
        UPDATE tblTask_AssignHistory
        SET StartDate = @StartDate, EndDate = @EndDate, Status = N'Pending', Progress = 0
        WHERE HeaderID = @NewHeaderID;
    END
    ELSE
    BEGIN
        INSERT INTO tblTask_AssignHeader (HeaderTitle, StartDate, PersonInCharge, Note)
        VALUES (@ParentTaskName, GETDATE(), @AssignedEmployeeID, NULL);
    ...
    BEGIN
        ...
        INSERT INTO tblTask_AssignHistory (HeaderID, EmployeeID, TaskID, StartDate, EndDate, Status, Progress)
        VALUES (@NewHeaderID, @MainResponsibleID, @ParentTaskID, @StartDate, @EndDate, N'Pending', 0);

        SELECT 1 AS Success, N'Giao việc thành công!' AS Message, @NewHeaderID AS HeaderID;
        RETURN;
    END
    ...
    BEGIN
        INSERT INTO tblTask_AssignHistory (
            HeaderID, EmployeeID, TaskID, StartDate, EndDate, Status, Progress, AssignPriority
        )
        SELECT
            @NewHeaderID,
            EmployeeID,
...
-- 1. Lấy dữ liệu khởi tạo cho form Giao việc
...
AS
BEGIN
	-- Table 1: Danh sách Header (Đợt giao việc)
    SELECT HeaderID, HeaderTitle FROM tblTask_AssignHeader ORDER BY StartDate DESC;

    -- Table 2: Danh sách Task (Chỉ lấy những task có thể làm cha hoặc độc lập)
    SELECT
        T.TaskID,
        T.TaskName,
        -- Description giả
    ...
    WHERE T.Status = 1;
        ...
        -- Table 03: Danh sách nhân viên
        DECLARE @getdate DATE = GETDATE();
        -- Lấy thông tin nhân viên hiện tại (giữ nguyên)
        -- Nếu truyền @ParentTaskID thì loại trừ những nhân viên đã có task đó trong lịch sử
        SELECT
                EmployeeID,
                FullName,
                PositionID
        ...
        WHERE EmployeeStatusID <> 20
            AND (
                ...
                OR EmployeeID NOT IN (
                        SELECT EmployeeID FROM tblTask_AssignHistory WHERE TaskID = @ParentTaskID
                )
            );
...
BEGIN
    ...
    INSERT INTO tblTask_AssignHistory (
        HeaderID, EmployeeID, TaskID, StartDate, EndDate, Status, Progress, ActualKPI
    )
    SELECT
        NULL,
        @EmployeeID,
...
-- =============================================
-- sp_Task_GetDetail - Thêm thông tin người yêu cầu và trách nhiệm chính
-- =============================================
...
BEGIN
    ...
    -- 1. Task chính + Thông tin người yêu cầu & trách nhiệm chính
    SELECT
        T.*,
        ...
        -- Thông tin từ AssignHistory của user hiện tại
        AH.HeaderID,
        AH.StartDate AS MyStartDate,
        AH.EndDate AS MyDueDate,

        -- Thông tin Header (nếu có)
        H.PersonInCharge AS RequestedBy,
        H.HeaderTitle,

        -- Lấy tên người yêu cầu từ PersonInCharge
        ISNULL(
        ...
        -- Nếu có Header -> lấy người được giao task cha (ParentTask) trong cùng Header
        -- Nếu không có Header -> lấy người được giao task này
        ISNULL(
            CASE
                WHEN AH.HeaderID IS NOT NULL THEN
                    -- Tìm task cha trong header này
                    (SELECT TOP 1 MainAH.EmployeeID
                     FROM tblTask_AssignHistory MainAH
                     INNER JOIN tblTask_Template TT ON TT.ChildTaskID = @TaskID
                     WHERE MainAH.HeaderID = AH.HeaderID
    ...
    LEFT JOIN tblTask_AssignHeader H ON H.HeaderID = AH.HeaderID
    WHERE T.TaskID = @TaskID
    ORDER BY AH.StartDate DESC;

    -- 2. Assign history với tên người thực hiện
    SELECT
        AH.HistoryID,
        AH.HeaderID,
        AH.EmployeeID,
    ...
    -- 3. Comments với tên người comment
    SELECT
        C.CommentID,
      C.EmployeeID,
        C.Content,
    ...
    -- 4. Checklist
    SELECT ItemID, Title, IsDone, SortOrder
    FROM tblTask_Checklist
    WHERE TaskID = @TaskID
    ORDER BY SortOrder;

    -- 5. Attachments với tên người upload
    SELECT
        A.AttachID,
        A.FileName,
        A.FilePath,
    ...
    -- 6. TASK CON HIỆN TẠI (Đã được giao) với tên người thực hiện
    SELECT
        TT.ChildTaskID,
    ...
    LEFT JOIN tblTask_AssignHistory H ON H.TaskID = T.TaskID AND H.EmployeeID = @EmployeeID
    WHERE TT.ParentTaskID = @TaskID
    ORDER BY TT.ChildTaskID;

    -- 7. DANH SÁCH TASK KHẢ DỤNG ĐỂ THÊM LÀM TASK CON
    SELECT
        T.TaskID,
        T.TaskName,
        ISNULL(T.KPIPerDay, 0) AS DefaultKPI,
    ...
    FROM tblTask T
    WHERE T.Status = 1
      AND T.TaskID != @TaskID -- Không phải chính nó
      AND T.PositionID IS NULL -- Không cố định theo chức vụ
      AND NOT EXISTS ( -- Chưa là task con của task khác
          SELECT 1 FROM tblTask_Template TT WHERE TT.ChildTaskID = T.TaskID
      )
      AND NOT EXISTS ( -- Không phải là task cha
          SELECT 1 FROM tblTask_Template TT WHERE TT.ParentTaskID = T.TaskID
      )
      AND NOT EXISTS ( -- Chưa được giao tự động
          SELECT 1 FROM tblTask_AssignHistory H
          WHERE H.TaskID = T.TaskID AND H.HeaderID IS NULL
      )
    ORDER BY T.TaskName;
END
GO
...
BEGIN
    SELECT
        ...
        T.Unit,
        ISNULL(T.Priority, 3) AS Priority,
        0 AS IsNew, -- Đánh dấu là task có sẵn
        ISNULL(TMP.SortOrder, 999) AS SortOrder -- Thêm SortOrder
    FROM tblTask_Template TMP
    INNER JOIN tblTask T ON TMP.ChildTaskID = T.TaskID
    WHERE TMP.ParentTaskID = @ParentTaskID
    ORDER BY ISNULL(TMP.SortOrder, 999), TMP.ChildTaskID; -- Sắp xếp theo SortOrder
END
GO
...
ALTER PROCEDURE [dbo].[sp_Task_UpdateSubtaskOrder]
    @ParentTaskID BIGINT,
    @OrderedChildIDs VARCHAR(MAX), -- Comma-separated: "10,11,12,13"
    @LoginID INT = NULL
AS
BEGIN
    SET NOCOUNT ON;

    -- Kiểm tra parent task tồn tại
    ...
    -- Parse danh sách child IDs và tạo bảng tạm với thứ tự
    ...
    INSERT INTO @TempOrder (ChildTaskID, SortOrder)
    SELECT
        CAST(value AS BIGINT) AS ChildTaskID,
        @Counter + (ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) - 1) AS SortOrder
    FROM STRING_SPLIT(@OrderedChildIDs, ',')
    WHERE LTRIM(RTRIM(value)) <> ''
      AND ISNUMERIC(value) = 1;

    -- Kiểm tra tất cả child tasks có thuộc parent này không
    IF EXISTS (
        SELECT 1
        FROM @TempOrder T
        WHERE NOT EXISTS (
            ...
            WHERE TT.ParentTaskID = @ParentTaskID
              AND TT.ChildTaskID = T.ChildTaskID
        )
    )
    BEGIN
        SELECT 0 AS Success, N'Một số task con không thuộc task cha này!' AS ErrorMessage;
        RETURN;
    END

    -- Cập nhật thứ tự trong bảng Template
    -- (Nếu bảng tblTask_Template có cột SortOrder)
    IF EXISTS (
        SELECT 1
        FROM sys.columns
        WHERE object_id = OBJECT_ID('tblTask_Template')
          AND name = 'SortOrder'
    )
    BEGIN
        UPDATE TT
        SET TT.SortOrder = T.SortOrder
        FROM tblTask_Template TT
        INNER JOIN @TempOrder T ON TT.ChildTaskID = T.ChildTaskID
        WHERE TT.ParentTaskID = @ParentTaskID;
    END
    ELSE
    BEGIN
        -- Nếu chưa có cột SortOrder, thêm vào
        IF NOT EXISTS (
            SELECT 1
            FROM sys.columns
            WHERE object_id = OBJECT_ID('tblTask_Template')
        ...
        BEGIN
            ALTER TABLE tblTask_Template ADD SortOrder INT NULL;
        END
    ...
    SELECT
        1 AS Success,
        N'Cập nhật thứ tự thành công!' AS Message,
        @ParentTaskID AS ParentTaskID,
        (SELECT COUNT(*) FROM @TempOrder) AS UpdatedCount;
END
GO