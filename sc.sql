...
function checkAndInitOnTabSelect(menuId, initFunction) {

     const tabButton = document.getElementById(''dynamic-tab-'' + menuId);
     if (tabButton) {
         // Tạo MutationObserver để theo dõi thay đổi thuộc tính
         const observer = new MutationObserver(function(mutations) {
       mutations.forEach(function(mutation) {
                if (mutation.type === ''attributes'' && mutation.attributeName === ''aria-selected'') {
                     const isSelected = tabButton.getAttribute(''aria-selected'') === ''true'';
                     if (isSelected) {
                         // Kiểm tra nếu initFunction là string thì chuyển thành function
	...
	// Linh : Hàm chung để save cột dữ liệu trong bảng
    function saveDataTableCommon(tableSN, columnNameSN, values, idColumnNameSN, idValue, onSuccess, onError) {
        const dataJSON = JSON.stringify([tableSN, columnNameSN, values.map((v) => String(v || "")), idColumnNameSN]);

        let idValuesJSON = null;
        if (idValue !== null && idValue !== undefined && String(idValue).trim() !== "") {
            idValuesJSON = JSON.stringify([[String(idValue)], idColumnNameSN]);
        }
	...
	// Linh : Hàm chung để dùng cho hpaControlText autosave
	function hpaControlEditTableRowAutoSave(event, tableSN, columnNameSN, idColumnSN) {
		event.stopPropagation();
	...
	// Linh : Hàm chung để dùng cho hpaControlText NO autosave
	function hpaControlEditTableRowNoAutoSave(event, options) {
		...
		if (isClickOnNewCell) {
			...
			// Adjust popup position if near bottom
			const $actions = $el.find(".edit-actions");
			const rect = $el[0].getBoundingClientRect();
			const spaceBelow = window.innerHeight - rect.bottom;
			if (spaceBelow < 60) {
			...
			$(document).off("click.hpaEdit").on("click.hpaEdit", function(e){
				hpaControlEditTableRowNoAutoSave(e, options);
			});
		...
		if (event.type === "click") {
			...
			if ($target.hasClass("btn-save")) {
				...
				$el.find(".editable-content").show().text(newVal);
				_hpa_close_editing($el);
				$(document).off("click.hpaEdit");

				// Save to pending changes (NOT save to DB yet)
				if (newVal !== curVal) {
					const id = $el.data("id");
					const col = $el.data("col");
					if (!pendingChanges[id]) pendingChanges[id] = {};
					pendingChanges[id][col] = newVal;
				}
			}
			// BUTTON CANCEL
			else if ($target.hasClass("btn-cancel")) {
...
function hpaControlDateBox(el, config) {
	...
	const cfg = {
		...
		tableSN: config.tableSN || config.tableName,
		...
		getValue: config.getValue || (() => $el.text().trim()),
		setValue: config.setValue || ((val) => $el.text(val))
	};
	...
	$el.on("click.datebox", function (e) {
		// Đóng các editor khác đang mở
		$(".hpa-editable-row.editing, .hpa-editable-row-number.editing")
			.not($el)
			.find(".btn-save")
			.trigger("click");
		...
		if ($el.hasClass("editing-date")) return;

		const rawText = cfg.getValue();
		const dateValue = parseDate(rawText);

		const $editorContainer = $("<div class=''dx-field-value'' style=''width:auto; height:100%''></div>");
		...
		const finish = () => {
			...
			const selectedDate = vnResult;

			// Cập nhật UI tạm thời
			cfg.setValue(selectedDate);
			...
			const dataJSON = JSON.stringify([
				...
				[cfg.columnName],
				[String(selectedDate || "")],
				[cfg.type === "datetime" ? "datetime" : "date"]
			]);

			const hasValidId = cfg.idValue !== null &&
							   cfg.idValue !== undefined &&
							   String(cfg.idValue).trim() !== "";

			const idValuesJSON = hasValidId
				? JSON.stringify([[String(cfg.idValue)], cfg.idColumnName])
				: null;
			...
			AjaxHPAParadise({
				...
				success: function(res) {
					try {
						...
						const newIdRow = results.find(r => !cfg.idValue && r.IDValue);
						const returnedId = newIdRow ? newIdRow.IDValue : cfg.idValue;

						cfg.setValue(formatDateSafe(selectedDate));
						cfg.onSave?.(selectedDate, false, returnedId);

					} catch (err) {
						console.error(err);
						uiManager.showAlert({
							type: "error",
							message: "Lỗi xử lý kết quả!"
						});
						cfg.setValue(rawText);
					}
				},
				error: function() {
					uiManager.showAlert({
		...
		const dxBox = $editorContainer.dxDateBox({
			...
			onKeyDown: function (e) {
				(e.event.key === "Enter" || e.event.key === "Escape") && e.component.close();
			}
		}).dxDateBox("instance");
