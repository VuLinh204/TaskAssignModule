...
BEGIN
    ...
    //Khởi tạo giao diện và Cache
        ...
        $(`<style>
            .hpa-file-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(110px, 1fr)); gap: 12px; margin-top: 15px; }
            .hpa-dropzone-modern { border: 2px dashed #e0e0e0; border-radius: 16px; padding: 12px; text-align: center; cursor: pointer; background: #fafafa; transition: all 0.3s ease; position: relative; overflow: hidden; }
            ...
            .file-card { background: #fff; border: 1px solid #eee; border-radius: 12px; position: relative; overflow: hidden; transition: all 0.2s; box-shadow: 0 2px 5px rgba(0,0,0,0.03); }
           ...
           .glass-toolbar { height: 60px; flex-shrink: 0; display: flex; align-items: center; justify-content: space-between; padding: 0 20px; background: rgba(0, 0, 0, 0.4); border-bottom: 1px solid rgba(255, 255, 255, 0.1); }
            ...
            .file-name { color: #fff; font-size: 14px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
            ...
            try {
                ...
                if (f.data && f.data.length > 50) {
                    ...
                    const mime = f.ext === "pdf" ? "application/pdf" :
                         ["mp4", "webm", "mov"].includes(f.ext) ? ("video/" + (f.ext === "mov" ? "quicktime" : f.ext)) :
                         ["mp3", "wav", "m4a"].includes(f.ext) ? ("audio/" + (f.ext === "m4a" ? "mp4" : f.ext)) :
                         ["pptx", "ppt"].includes(f.ext) ? "application/vnd.openxmlformats-officedocument.presentationml.presentation" :
                ...
                const thumbStrategies = {
                    ...
                    pptx: async () => {
                        ...
                        try {
                            // Load file zip
                            const zip = await JSZip.loadAsync(ab);
                            // Tìm file thumbnail mặc định của Office Open XML
                            const thumbFile = zip.file("docProps/thumbnail.jpeg");

                            if (thumbFile) {
                                const bg = await thumbFile.async("base64");
                                const url = "data:image/jpeg;base64," + bg;
                                return await drawCanvasPreview(null, null, null, null, url);
                            }
                        } catch (e) { }
                        return null;
                    },
    ...
    //Hàm hiển thị Modal Preview (Web)
        ...
        async function preview_%ColumnName%(rawF) {
            ...
            const renderStrategies = {
                ...
                audio: async (url) => {
                    $v.html(`
                        <div style="text-align:center; padding: 40px; background: rgba(255,255,255,0.1); border-radius: 16px; width: 100%; max-width: 400px;">
                            <i class="bi bi-music-note-beamed" style="font-size: 64px; color: #fff; margin-bottom: 20px; display: block;"></i>
                ...
                ppt: async (url) => {
                    $v.html(`
                        <div style="text-align:center; padding: 40px; background: #fff; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); width: 100%; max-width: 450px;">
                            <i class="bi bi-file-earmark-slides-fill" style="font-size: 80px; color: #f97316; margin-bottom: 20px; display: block;"></i>
                            <h3 style="margin: 0 0 10px 0; color: #333; font-size: 18px; word-break: break-word;">${f.name}</h3>
                            <p style="color: #666; font-size: 14px; margin-bottom: 25px;">Trình duyệt chưa hỗ trợ xem trước Slide trực tuyến.<br>Vui lòng tải về để xem nội dung.</p>
                            <a href="${url}" download="${f.name}" style="display: inline-flex; align-items: center; justify-content: center; gap: 8px; background: #f97316; color: white; text-decoration: none; padding: 10px 24px; border-radius: 6px; font-weight: 500; transition: background 0.2s;">
                                <i class="bi bi-download"></i> Tải về ngay
                            </a>
                        </div>
                    `);
                },
            ...
            try {
                ...
                else if (["mp4", "webm", "mov"].includes(f.ext)) type = "video";
                else if (["mp3", "wav", "m4a"].includes(f.ext)) type = "audio";
                else if (["docx", "doc"].includes(f.ext)) type = "docx";
                else if (["xlsx", "xls"].includes(f.ext)) type = "xlsx";
                else if (["pptx", "ppt"].includes(f.ext)) type = "ppt";
    ...
    //Hàm Lazy Load Preview
        async function loadRealPreview_%ColumnName%(targetElem, f) {
            ...
            try {
                ...
                else if (f.isImg || ["mp4", "webm", "mov", "pdf", "docx", "doc", "xlsx", "xls", "txt"].includes(f.ext)) {
                      const cacheKey = (f.url || f.name).replace(/[^a-zA-Z0-9]/g, "_");
        ...
        function render_%ColumnName%() {
            ...
            files.forEach(rawF => {
                ...
                const iconMap = {
                    ...
                    pptx: {icon: "bi-file-earmark-slides", color: "#f97316"}, ppt: {icon: "bi-file-earmark-slides", color: "#f97316"}, // <--- Thêm PowerPoint
                    txt: {icon: "bi-file-text", color: "#6b7280"},
                    mp3: {icon: "bi-file-earmark-music", color: "#0ea5e9"}, wav: {icon: "bi-file-earmark-music", color: "#0ea5e9"}, m4a: {icon: "bi-file-earmark-music", color: "#0ea5e9"}, // <--- Thêm Audio
                    mp4: {icon: "bi-camera-video", color: "#9333ea"},
                    webm: {icon: "bi-camera-video", color: "#9333ea"},
                    mov: {icon: "bi-camera-video", color: "#9333ea"}
                };
  ...
  configurable: true, get: function() { return _localData_%ColumnName%; },
        ...
        instanceUploader = $("#uploader_" + colName).dxFileUploader({
            ...
            onValueChanged: () => { 
                ...
                if (_autoSave%ColumnName%%UID%) {
                    timeOutSave = setTimeout(save_%ColumnName%, 150); 
                }
            }
        ...
        if (_readOnly%ColumnName%%UID%) {
             $("#dropzone_" + colName).hide();
             $("#trigger_dropzone_" + colName).hide();
             $("<style>#filelist_" + colName + " .btn-del { display: none !important; }</style>").appendTo("head");
             if (instanceUploader) instanceUploader.option("disabled", true);
        }
    ...
    //Hàm lưu file
        ...
        render_%ColumnName%();
    '
    WHERE [Type] = 'hpaControlFile' AND ReadOnly = 0
END
GO