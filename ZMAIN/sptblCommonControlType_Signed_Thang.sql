...
BEGIN
  ...
  %IDDiv%Instance._resumeValueChangeAction()'
    ...
    -- Linh: XỬ LÝ GRID (hpaControlGrid)
        -- Extract DXBoxUI
        IF OBJECT_ID('tempdb..#DXBoxUI') IS NOT NULL DROP TABLE #DXBoxUI;
        SELECT
            t.ID, t.ColumnName, t.Type, t.DisplayName, t.GridColumnName, t.AutoSave, t.ReadOnly, t.SPLoadData, t.TableEditor, t.columnId,
            DXBoxUI = SUBSTRING(
                t.loadUI,
                ...
                loadUI = N'
                    ...
                    function loadUI ' + @currentGrid + N'() {
                        ...
                        const store = new DevExpress.data.ArrayStore({
                            data: ' + @currentGrid + N'
                            DataSource,
                            key: "' + @gridPrimaryKey + N'"
                        });
                        /*BEGIN_DX*/
                        ' + @currentGrid + N'
                        Instance = $("<div>").appendTo($container).dxDataGrid({
                            dataSource: store,
                            ...
                            masterDetail: {
                                ...
                                template: function(container, options) {
                                    ...
                                    AllData ? window.
                                    ' + @currentGrid + N'
                                    AllData.filter(function(t) {
                                        return t.ParentTaskID === parentTaskId;
                                    }): [];
                            ...
                            onRowUpdating: async function(e) {
                                ...
                                try {
                                    const dataJSON = JSON.stringify(["%tableId%", [col],
                                        [newVal]
                                    ]);
                                    const idValuesJSON = JSON.stringify([
                                        [e.key], "' + @gridColumnId + N'"
                                    ]);
                                    const json = await saveFunction(dataJSON, idValuesJSON);
                                    const dtError = json.data[json.data.length - 1] || [];
                                    if (dtError.length > 0 && dtError[0].Status === "ERROR") {
                                    ...
                                    const rowIdx = ' + @currentGrid + N'
                                    Instance.getRowIndexByKey(e.key);
                                    ' + @currentGrid + N'
                                    Instance.cellValue(rowIdx, col, newVal);
                        ...
                        return {
                            setValue: val => {
                                ...
                                if (' + @currentGrid + N'
                                    Instance) {
                                    ' + @currentGrid + N'
                                    Instance.option("dataSource", ' + @currentGrid + N'
                                        DataSource);
                                    ' + @currentGrid + N'
                                    Instance.refresh();
                                }
                            },
                            getValue: () => ' + @currentGrid + N'
                            DataSource,
                            getInstance: () => ' + @currentGrid + N'
                            Instance
                        };
                    }  
                ',
