...
BEGIN
    ...
    SET @html = N'
        ...
        <script>
                ...
                tasks.forEach((task) => {
                    ...
                    <tr>
                        ...
                        <td class="editable position-relative" data-id="${task.TaskID}" data-col="TaskName">
                            ...
                            <template class="editFormTemplate" onclick="hpaControlEditTableRow(event)" style="display: block; position: absolute; inset: 0; height: 100%; z-index: 1; cursor: pointer;">
                                <div class="position-relative w-100">
                                    <input type="text" class="form-control form-control-sm edit-input w-100 rounded py-1 px-2" 
                                        ...
                                        oninput="hpaControlEditTableRow(event)" 
                                        onkeydown="hpaControlEditTableRow(event)">
                                    <div class="edit-actions" style="position: absolute; top: 110%; right: 0; display: flex; justify-content: flex-end; gap: 4px; align-items: center; z-index: 100; padding: 6px 8px; border-radius: 6px; box-shadow: 0 4px 15px rgba(0,0,0,0.15);">
                                        <button type="button" class="btn-save btn btn-sm" onclick="hpaControlEditTableRow(event)" style="width: 28px; height: 28px; padding: 0; display: inline-flex; align-items: center; justify-content: center; border-radius: 4px !important; border: 1px solid #e8eaed; background: #2E7D32; color: white; cursor: pointer; transition: all 0.2s; font-size: 14px;" title="Lưu"><i class="bi bi-check-lg"></i></button>
                                        <button type="button" class="btn-cancel btn btn-sm" onclick="hpaControlEditTableRow(event)" style="width: 28px; height: 28px; padding: 0; display: inline-flex; align-items: center; justify-content: center; border-radius: 4px !important; border: 1px solid #e8eaed; backdrop-filter: blur(50px); cursor: pointer; transition: all 0.2s; font-size: 14px;" title="Hủy"><i class="bi bi-x-lg"></i></button>
                                    </div>
                                </div>
                            </template>
                        </td>
            ...
            function hpaControlEditTableRow(event, options) {
                ...
                if (isClickOnNewCell) {
                    ...
                    $(document).off("click.hpaEdit").on("click.hpaEdit", function(e){
                        hpaControlEditTableRow(e, options);
                    });
